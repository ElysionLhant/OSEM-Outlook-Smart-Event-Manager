...
*   **模板编辑器：** 打开配置中心（模板、Prompt、脚本、本地文件仓库地址）。
*   **完结选中项：** 归档选中的事件。
...
*   **文件区：** 此事件专用的本地文件夹（每个事件独立）。
    ...
    *   **生成文件夹：** 在“模板编辑器”中配置的“本地文件仓库地址”下，为当前事件创建一个同名文件夹。
...
*   **选择范围：**
    ...
    *   **日期范围：** 根据最后更新日期筛选事件。
    *   **事件状态：** 选择导出 **已归档 (Archived)** 事件、**进行中 (Ongoing)** 事件，或两者都导出。
*   **内容选项：**
    *   **仪表盘数据：** 导出结构化数据 (CSV)。
    *   **附件：** 导出邮件附件。
    *   **额外文件：** 导出事件本地文件夹中的文件。
        *   **智能模板文件导出：** OSEM 会自动计算事件文件夹中模板文件的 MD5 哈希值。如果文件未被修改（哈希值与原始模板一致），导出时将自动跳过以节省空间。仅导出被修改过的文件。
*   **文件类型：** 筛选要导出的文件扩展名（如仅导出 PDF）。
    *   **事件跨度：** 选择导出特定日期范围内的事件。
*   **导出内容：**
